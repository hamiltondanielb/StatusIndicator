...
- (void)openPanel
{
    ...
    [fb observeEventType:FEventTypeValue withBlock:^(FDataSnapshot *snapshot) {
        if(snapshot.value == [NSNull null]) {
            ...
            NSError * error;
            //[NSData dataWithContentsOfFile:snapshot.value
            NSData *data = [NSJSONSerialization dataWithJSONObject:snapshot.value options:0 error:&error];
            NSMutableDictionary  * json = [NSJSONSerialization JSONObjectWithData:data
                                                                           options:0
                                                                             error:&error];
            
            NSLog(@"%@",json[@"builds"]);
            NSArray *builds =json[@"builds"];
            NSMutableString *list = [[NSMutableString alloc]init];
            for(int i = 0; i < builds.count; i++){
                NSLog(@"%@",json[@"builds"][i][@"number"]);
                [list appendString:json[@"builds"][i][@"number"]];
                [list appendString:@"\n"];
                [list appendString:json[@"builds"][i][@"description"]];
                [list appendString:@"\n"];
                [list appendString:@"\n"];
            }
            [self.textField setStringValue:[NSString stringWithString:list]];
        }
    }];
     
    [NSAnimationContext beginGrouping];
    [[NSAnimationContext currentContext] setDuration:openDuration];
    [[panel animator] setFrame:panelRect display:NO];
